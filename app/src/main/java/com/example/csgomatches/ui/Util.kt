...
fun formatDate(date: Date?, context: Context): String? {
    ...
    val hour = SimpleDateFormat(" HH:mm").apply { timeZone = TimeZone.getDefault() }.format(date)
    ...
    return SimpleDateFormat("dd.MM HH:mm").apply { timeZone = TimeZone.getDefault() }.format(date)
}